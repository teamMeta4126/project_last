...
// Resetting the distributed matrix allows utilities to propagate through already distributed zones (only distributing to the needed zones).
                ...
                while (cellQueue.isEmpty() == false) {
                    ...
                    if (currentCell instanceof Zone) {
                        ...
                        boolean hasCurrentNeed = false;

                        if (currentProvider instanceof WaterPumping && currentZone.hasWater() == false) {
                            hasCurrentNeed = true;
                        } else if (currentProvider instanceof PowerPlant && currentZone.hasElectricity() == false) {
                            hasCurrentNeed = true;
                        } else if (currentProvider instanceof InternetHub && currentZone.hasInternet() == false) {
                            hasCurrentNeed = true;
                        }

                        if (hasCurrentNeed == true) {
                            if (utilityAmount >= currentDemand) {
                                utilityAmount = utilityAmount - currentDemand;

                                if (currentProvider instanceof InternetHub) {
                                    currentZone.setHasInternet(true);
                                    printUtilityOutput(currentZone, "internet", currentDemand);
                                } else if (currentProvider instanceof WaterPumping) {
                                    currentZone.setHasWater(true);
                                    printUtilityOutput(currentZone, "water", currentDemand);
                                } else if (currentProvider instanceof PowerPlant) {
                                    currentZone.setHasElectricity(true);
                                    printUtilityOutput(currentZone, "electricity", currentDemand);
                                }
                            }
                        }
                    }
